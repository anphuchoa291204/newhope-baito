...
const StudentImport = () => {
    ...
    const handleImportFromExcel = async () => {
        ...
        try {
            ...
            const studentsToImport = excelData.slice(1).map((row, index) => ({
                index: index + 2, // Line number in Excel (including header)
                fullname: row[0] || "", // Full Name
                email: row[1] || "", // Email
                date_of_birth: row[2] || "", // Date of Birth
                gender: row[3] || "", // Gender
                phone_number: row[4] || "", // Phone Number
                nationality: row[5] || "", // Nationality
                major: row[6] || "", // Major
                japan_skill: row[7] || "", // Japanese
                otherLanguages: row[8] || "", // Other Languages
            }));
            ...
            studentsToImport.forEach((student) => {
                ...
                if (email && !email.toLowerCase().match(EMAIL_REGEX)) rowErrors.push("Invalid email" +
                    " address." +
                    " Please check your email address again.");
                if (!date_of_birth) rowErrors.push("Date of Birth is missing.");
                if (serialDateToExcelDate(date_of_birth) === "Invalid Date" || isMatch(format(serialDateToExcelDate(date_of_birth), "dd/MM/yyyy"), "dd/MM/yyyy") && !isValid(new Date(format(serialDateToExcelDate(date_of_birth), 'yyyy/MM/dd')))) rowErrors.push("Invalid" +
    ...
    return <Stack spacing={2} direction="column" sx={{width: "100%", height: "100%"}}>
        <Stack spacing={2} direction="row" alignItems="center" flexWrap="wrap" gap={2} useFlexGap>
            ...
            <Button variant="contained" color="success" onClick={handleImportFromExcel}
